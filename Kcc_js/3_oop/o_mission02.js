...
class MaleIdol extends Idol {
  sing = () => ` ${super.name}이 노래를 합니다.`;
}

class FemaleIdol extends Idol {
  dance = () => `${super.name}이 춤을 춥니다.`;
}
...
const btsMembersArr = btsMembers.map((el) => new MaleIdol({ ...el }));
const bts = new IdolGroup('BTS', btsMembersArr);

const iveMembersArr = iveMembers.map((el) => new FemaleIdol({ ...el }));
const ive = new IdolGroup('IVE', iveMembersArr);

const kor = new Country('한국', [bts, ive]);
