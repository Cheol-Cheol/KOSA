...
public class ShoppingMall {
	...
	public void addCartItem(int productId) {
		...
		boolean found = productList.stream().filter(product -> product.getProductId() == productId).peek(product -> {
	...
	public void getFilterProductList(int categoryIdx) {
		...
		productList.stream().filter(product -> product.getCategory().name().equals(category.name()))
				.forEach(Product::printProductInfo);
	...
	public void getOrderListByUser() {
		...
		loginUser.getOrderList().stream().forEach(Order::printOrderInfo);
	...
	public void getCartListByUser() {
		...
		loginUser.getCart().stream().forEach(Product::printProductFromCart);
		int totalPrice = loginUser.getCart().stream().mapToInt(Product::getPrice).sum();
	...
	public void login(String nickname) {
		...
		boolean found = userList.stream().filter(user -> user.getName().equals(nickname.trim())).peek(user -> {
			loginUser = user;
			System.out.println("------------------------------------------------------------------------------------");
		...
		if (!found) {
			System.out.println("[서비스 알림] 로그인 실패");
		}
	...
	public void getUserList() {
		...
		userList.stream().forEach(User::printUserInfo);
	...
	public void getProductList() {
		...
		productList.stream().forEach(Product::printProductInfo);
